...
namespace SearchFunc.AzureSearchFunc
{
    static class WebApiSkillHelpers
    {
        ...
        public static async Task<WebApiSkillResponse> ProcessRequestRecordsAsync(string functionName, IEnumerable<WebApiResponseRecord> requestRecords, Func<WebApiRequestRecord, WebApiRequestRecord, Task<WebApiRequestRecord>> processRecord)
        {
            WebApiSkillResponse response = new WebApiSkillResponse();
            
            foreach (WebApiResponseRecord inRecord in requestRecords)
            {
                WebApiResponseRecord outRecord = new WebApiResponseRecord() { RecordId = inRecord.RecordId };
                try
                {
